...
library(DT)
library(plotly)
# writexl : export Excel des quotas enregistrés (2026-09) — à ajouter au
# déploiement (manifest / renv) ; appelé avec writexl:: dans le serveur.
...
modele_reference <- function(r, cascade, modeles = NULL) {
  ...
  for (k in cascade) {
    # Modeles « quota de depart » (quota_depart = TRUE dans le registre, ex.
    # Vaillancourt et Boivin 2000) : leur valeur est deja un quota fixe, pas un
    # rendement maximal soutenu. Jamais retenus comme reference -- sinon le
    # taux de 80 % s'appliquerait par-dessus un quota deja prudent.
    if (!is.null(modeles) && isTRUE(modeles[[k]]$quota_depart)) next
    res <- r[[k]]
...
#' Fusionner l'onglet Lacs par lac (nolac) — priorité IPE > OG > autre,
#' VARIABLE PAR VARIABLE (révision 2026-09).
#'
#' Plusieurs UE peuvent exister pour un même lac (ex. « 08-00727-IPE » et
#' « 08-00727-OG »). Avant : la ligne IPE était retenue en bloc ; si elle
#' n'avait pas de profondeur moyenne, la valeur présente dans la ligne OG
#' était perdue et le champ restait vide. Maintenant, pour chaque variable, on
#' prend la première valeur non manquante dans l'ordre IPE -> OG -> autre, et
#' on conserve l'UE d'origine dans une colonne « <variable>_ue » (affichée
#' sous les champs).
#'
#' Hypothèse : la morphométrie (superficie, profondeurs, périmètre) et les
#' coordonnées sont des caractéristiques du LAC, indépendantes de l'inventaire
#' qui les a consignées. Une ligne peut donc combiner des valeurs issues d'UE
#' différentes. Si deux UE donnent des valeurs discordantes, seule celle de
#' rang supérieur est retenue (aucune moyenne) — la cohérence entre variables
#' (ex. prof. moy. < prof. max) reste contrôlée en aval (msg_morpho).
#'
#' @return une ligne par nolac ; ue_min = UE de rang le plus élevé du lac ;
#'   colonnes <variable>_ue pour les variables de VARIABLES_UE_SUIVIES.
VARIABLES_UE_SUIVIES <- c("sup", "prof_max", "prof_moy", "perimetre", "lat", "lon")

dedup_lacs_par_ue <- function(df_lacs) {
  if (!("nolac" %in% names(df_lacs))) return(df_lacs)
  df <- df_lacs %>% dplyr::filter(!is.na(nolac))
  if (!("ue_min" %in% names(df))) df$ue_min <- rep(NA_character_, nrow(df))

  suffixe <- toupper(trimws(sub(".*-", "", df$ue_min)))
  df$.rang_ue <- dplyr::case_when(
    suffixe == "IPE" ~ 1L,
    suffixe == "OG"  ~ 2L,
    TRUE             ~ 3L
  )

  cols_val   <- setdiff(names(df), c("nolac", "ue_min", ".rang_ue"))
  cols_suivi <- intersect(VARIABLES_UE_SUIVIES, cols_val)
  # Première valeur non manquante (x[NA] renvoie un NA du bon type)
  premiere <- function(x) x[which(!is.na(x))[1]]

  df <- df %>% dplyr::arrange(nolac, .rang_ue)

  # UE d'origine de chaque variable suivie — calculée AVANT le résumé des
  # valeurs, pendant que ue_min et les variables sont encore des vecteurs
  src <- df %>%
    dplyr::group_by(nolac) %>%
    dplyr::summarise(dplyr::across(dplyr::all_of(cols_suivi),
                                   ~ ue_min[which(!is.na(.x))[1]],
                                   .names = "{.col}_ue"),
                     .groups = "drop")

  df %>%
    dplyr::group_by(nolac) %>%
    dplyr::summarise(ue_min = dplyr::first(ue_min),
                     dplyr::across(dplyr::all_of(cols_val), premiere),
                     .groups = "drop") %>%
    dplyr::left_join(src, by = "nolac")
}

# =============================================================================
# SÉLECTION DES STATIONS — règle « station la plus profonde » (révision 2026-09 v2)
# =============================================================================
# Quand plusieurs stations ont été échantillonnées le même jour (ou dans le
# même inventaire), on ne fait PLUS de moyenne entre stations : on retient la
# station la plus « valide », soit la plus PROFONDE (plus grande profondeur
# lue dans l'onglet Profil). Justification : la station profonde est
# habituellement la station principale du lac ; c'est la seule qui décrit
# toute la colonne d'eau (stratification, thermocline, oxygène en profondeur),
# alors qu'une moyenne mélangerait des profils de bassins différents (une
# station peu profonde tire la thermocline vers le haut, par exemple).
# Égalité de profondeur : la station qui a le plus de lectures.
# Hypothèse à garder en tête : la station la plus profonde n'est pas
# forcément représentative d'un lac à plusieurs bassins distincts.

#' Clé de station comparable entre onglets (« 3 », « 3.0 » et « 03 » -> « 3 »)
cle_station <- function(x) {
  n <- suppressWarnings(as.numeric(as.character(x)))
  ifelse(is.na(n), trimws(as.character(x)), as.character(n))
}

#' Station la plus profonde d'un ensemble de lectures de profil.
#' @param d  lectures (colonnes station, prof_mes) d'une même date/inventaire
#' @return list(station = clé retenue ou NA si pas de colonne station,
#'   prof = profondeur max de cette station, n_stations = nb de stations)
station_la_plus_profonde <- function(d) {
  if (!("station" %in% names(d)) || all(is.na(d$station)))
    return(list(station = NA_character_,
                prof = suppressWarnings(max(d$prof_mes, na.rm = TRUE)), n_stations = 1L))
  st <- d %>%
    dplyr::filter(!is.na(prof_mes)) %>%
    dplyr::mutate(station_cle = cle_station(station)) %>%
    dplyr::group_by(station_cle) %>%
    dplyr::summarise(prof = max(prof_mes), n_lect = dplyr::n(), .groups = "drop") %>%
    dplyr::arrange(dplyr::desc(prof), dplyr::desc(n_lect))
  if (nrow(st) == 0) return(list(station = NA_character_, prof = NA_real_, n_stations = 0L))
  list(station = st$station_cle[1], prof = st$prof[1], n_stations = nrow(st))
}

#' Paramètre de l'onglet Profil lu près de 5 m (pH, conductivité).
#'
#' Règle confirmée (2026-09 v2) : profil le plus RÉCENT du lac ; parmi les
#' stations de cette date qui ont une lecture du paramètre, la PLUS PROFONDE ;
#' dans cette station, la lecture la plus proche de 5 m (aucune tolérance :
#' un lac de moins de 5 m donne sa lecture la plus profonde) — une seule
#' lecture, aucune moyenne. Si deux lectures sont à égale distance de 5 m
#' (ex. 4 m et 6 m), la moins profonde est retenue (plus proche de
#' l'épilimnion échantillonné en surface).
#'
#' @param df_lac lignes de l'onglet Profil pour UN lac
#' @param col    "ph" ou "cond"
#' @return list(valeur, date (Date), prof (m), station, n_stations)
PROF_CIBLE_PH <- 5
extraire_profil_5m <- function(df_lac, col, prof_cible = PROF_CIBLE_PH) {
  vide <- list(valeur = NA_real_, date = as.Date(NA), prof = NA_real_,
               station = NA_character_, n_stations = 0L)
  if (is.null(df_lac) || nrow(df_lac) == 0 ||
      !all(c(col, "prof_mes") %in% names(df_lac))) return(vide)
  d <- df_lac[!is.na(df_lac[[col]]) & !is.na(df_lac$prof_mes), ]
  if (nrow(d) == 0) return(vide)
  if (!("date" %in% names(d))) d$date <- rep(as.Date(NA), nrow(d))

  # Profil le plus récent (lectures sans date : seulement si aucune n'est datée)
  if (any(!is.na(d$date))) d <- d[!is.na(d$date) & d$date == max(d$date, na.rm = TRUE), ]

  st <- station_la_plus_profonde(d)
  if (!is.na(st$station)) d <- d[cle_station(d$station) == st$station, ]

  d <- d[order(abs(d$prof_mes - prof_cible), d$prof_mes), ]
  list(valeur     = round(d[[col]][1], 2),
       date       = d$date[1],
       prof       = d$prof_mes[1],
       station    = st$station,
       n_stations = st$n_stations)
}

#' Texte de provenance d'une lecture de profil (sous le champ)
texte_profil_5m <- function(x) {
  paste0(if (!is.na(x$date)) format(x$date, "%Y-%m-%d") else "date inconnue",
         ", à ", formatC(x$prof, format = "f", digits = 1, decimal.mark = ","), " m",
         if (!is.na(x$station)) paste0(" (station ", x$station, ")") else "")
}

#' Pivoter l'onglet Parametre (format long) : un scalaire par (nolac, code),
#' en retenant le résultat de la date la plus récente.
#'
#' Plusieurs stations à cette date (révision 2026-09 v2) : l'onglet Parametre
#' n'a pas de profondeur ; on retient la station la plus profonde d'après
#' l'onglet Profil À LA MÊME DATE (numéros de station appariés). Si aucune
#' des stations n'a de profil ce jour-là, la profondeur est inconnue et on
#' revient à la moyenne des stations (signalée sous le champ).
#'
#' @param profil onglet Profil déjà normalisé (facultatif)
#' @return data.frame(nolac, code_param, resultat, date, n_mesures, station,
#'   prof_station, methode) ou NULL si colonnes manquantes
pivoter_parametre <- function(df_param, profil = NULL) {
  ...
  a_station <- "station" %in% names(df_param) && any(!is.na(df_param$station))
  df_param$station_cle <- if (a_station) cle_station(df_param$station)
                          else rep(NA_character_, nrow(df_param))

  # Date la plus récente par (lac, paramètre). Mesures sans date : utilisées
  # seulement si aucune mesure datée n'existe pour ce lac et ce paramètre.
  df <- df_param %>%
  ...
  # Profondeur de chaque station, par lac et par date, depuis l'onglet Profil
  a_prof <- a_station && !is.null(profil) &&
            all(c("nolac", "date", "station", "prof_mes") %in% names(profil))
  if (a_prof) {
    prof_st <- profil %>%
      dplyr::filter(!is.na(nolac), !is.na(date), !is.na(station), !is.na(prof_mes)) %>%
      dplyr::mutate(station_cle = cle_station(station)) %>%
      dplyr::group_by(nolac, date, station_cle) %>%
      dplyr::summarise(prof_station = max(prof_mes), n_lect = dplyr::n(), .groups = "drop")
    df <- dplyr::left_join(df, prof_st, by = c("nolac", "date", "station_cle"))
  } else {
    df$prof_station <- rep(NA_real_, nrow(df))
    df$n_lect       <- rep(NA_integer_, nrow(df))
  }

  df %>%
    dplyr::group_by(nolac, code_param) %>%
    dplyr::mutate(n_st = dplyr::n(), prof_connue = any(!is.na(prof_station))) %>%
    # NA en dernier : la première ligne est la station la plus profonde
    dplyr::arrange(dplyr::desc(prof_station), dplyr::desc(n_lect), .by_group = TRUE) %>%
    dplyr::summarise(
      methode      = if (dplyr::first(n_st) == 1) "unique"
                     else if (dplyr::first(prof_connue)) "station_profonde" else "moyenne",
      resultat     = if (methode == "moyenne") round(mean(resultat), 2) else dplyr::first(resultat),
      date         = dplyr::first(date),
      n_mesures    = dplyr::first(n_st),
      station      = if (methode == "station_profonde") dplyr::first(station_cle) else NA_character_,
      prof_station = if (methode == "station_profonde") dplyr::first(prof_station) else NA_real_,
      .groups      = "drop") %>%
    dplyr::select(nolac, code_param, resultat, date, n_mesures, station, prof_station, methode)
}
...
extraire_param_date <- function(df_param_pivot, nolac_val, code) {
  ...
  met <- if ("methode" %in% names(ligne)) ligne$methode[1] else NA_character_
  paste0(format(as.Date(ligne$date[1]), "%Y-%m-%d"),
         if (identical(met, "station_profonde"))
           paste0(" (station ", ligne$station[1], ")")
         else if (!is.na(n) && n > 1)
           paste0(" (moy. de ", n, " stations)")
         else "")
}
...
# -----------------------------------------------------------------------------
# Vaillancourt et Boivin 2000 -- Omble de fontaine (pourvoiries du Saguenay -
# Lac-Saint-Jean)
#   Source : Vaillancourt_et_Boivin_2000.pdf (« Methodes utilisees pour
#   determiner les quotas de depart des lacs a omble de fontaine », mise a jour
#   du 11 avril 2000).
#
#   NATURE DU RESULTAT : un QUOTA DE DEPART (kg/ha/an) fixe par la methode, et
#   non un rendement maximal soutenu. Le taux recommande de 80 %
#   (PCT_RECOMMANDE, « pretty good yield ») ne s'y applique donc PAS : le
#   modele est marque quota_depart = TRUE dans le registre, ce qui
#     - l'exclut du choix du modele de reference (modele_reference()), et
#     - affiche sa valeur telle quelle (x superficie) dans la colonne
#       « Quota recommande » du tableau des modeles.
#   Il sert a la comparaison ; le modele recommande reste Valin-Vaillancourt 1998.
#
#   TABLE DE DECISION, et non cascade multiplicative : le PREMIER code
#   applicable, dans l'ordre du document, determine la valeur (decision
#   2026-09). Les codes sont mutuellement exclusifs une fois cet ordre impose.
#
#     Allo. / Meun.  Situation                         Allopatrie    Meunier
#     1  / 13        sans tributaire ni emissaire      0             0
#     2  / 14        superficie < 2 ha                 non considere (NA)
#     3  / 15        2 < superficie <= 5 ha            2             0,6
#     5  / 17        > 5 ha, prof. moyenne inconnue    1,5           0,5
#     7  / 19        prof. moyenne 1,5 a 2,5 m         2             0,6
#     9  / 21        prof. moyenne < 1,5 m             1             0,3
#     10 / 22        prof. moyenne connue (> 2,5 m)    70 % Vezina   21 % Vezina
#
#   Valeurs reprises TELLES QU'ECRITES dans le document. Le facteur meunier
#   vaut 30 % de la valeur allopatrique partout, sauf le code 17 (0,5 au lieu
#   de 0,45) : ecart conserve volontairement (decision 2026-09).
#
#   Codes NON implementes (decision 2026-09) :
#     - 4, 6, 8, 12 / 16, 18, 20, 24 (« rendement ajuste au terme de
#       plusieurs annees d'exploitation ») : aucun calcul defini, et
#       l'ajustement a posteriori vaut pour tout modele ;
#     - 11 / 23 (100 % / 30 % de Vezina) : le code de depart 10 / 22 est retenu.
#
#   HYPOTHESES D'INTERPRETATION (non precisees par le document) :
#     - bornes : « entre 1,5 et 2,5 m » inclusives ; > 2,5 m -> code 10/22 ;
#     - superficie EXACTEMENT 2 ha : ni « < 2 ha » ni « > 2 ha » -> lacune du
#       document, modele indisponible plutot que de choisir un code ;
#     - aucune borne superieure de profondeur moyenne dans le document ; au-dela
#       de 25,9 m (borne retenue par Vaillancourt 1998), avertissement seulement.
# -----------------------------------------------------------------------------
TABLE_BOIVIN_2000 <- data.frame(
  situation     = c("sans_tributaire", "sup_inf_2", "sup_2_5", "sup_5_dmn_inconnue",
                    "dmn_1_5_2_5", "dmn_inf_1_5", "dmn_connue"),
  libelle       = c("lac sans tributaire ni émissaire",
                    "superficie < 2 ha",
                    "superficie > 2 ha et ≤ 5 ha",
                    "superficie > 5 ha, profondeur moyenne inconnue",
                    "profondeur moyenne entre 1,5 et 2,5 m",
                    "profondeur moyenne < 1,5 m",
                    "profondeur moyenne connue"),
  code_allopatrie = c(1, 2, 3, 5, 7, 9, 10),
  code_meunier    = c(13, 14, 15, 17, 19, 21, 22),
  # type : "fixe" = kg/ha/an ; "pct_vezina" = fraction du rendement Vezina ;
  #        "non_considere" = le document exclut explicitement ces lacs
  type          = c("fixe", "non_considere", "fixe", "fixe", "fixe", "fixe", "pct_vezina"),
  allopatrie    = c(0, NA, 2,   1.5, 2,   1,   0.70),
  meunier       = c(0, NA, 0.6, 0.5, 0.6, 0.3, 0.21),
  stringsAsFactors = FALSE
)

# Classe d'especes Vaillancourt-Boivin 2000 (decision 2026-09). Le document ne
# distingue que deux situations :
#   - "allopatrie" : omble de fontaine seul ;
#   - "meunier"    : meunier (catostomes) present. Les menes/mulets sont
#                    toleres EN PLUS du meunier -- le document ne traite que la
#                    presence du meunier et ne les exclut pas.
# Toute autre espece (brochet, touladi et equivalents, epineux, menes SANS
# meunier, espece non repertoriee) -> NA : modele indisponible, jamais
# rattache par defaut a l'une des deux tables.
classer_classe_boivin <- function(grp) {
  if (grp$allopatrie) return("allopatrie")
  autres <- setdiff(grp$brutes,
                    c(ESPECES_CATOSTOMES_OMBLE, ESPECES_CYPRINS_OMBLE, "allopatrie"))
  if (grp$catostomes && length(autres) == 0) return("meunier")
  NA_character_
}

# @param A                 Superficie (ha) -- intrant bloquant (les codes de
#   superficie precedent ceux de profondeur : sans elle, impossible de savoir
#   si les codes 2/3 s'appliquent)
# @param grp               Liste de booleens -- especes_groupes_omble()
# @param tributaire_absent TRUE = absence confirmee, FALSE = present, NA = inconnu
# @param Dmn_optionnel     Profondeur moyenne (m), NA si inconnue. NON bloquante :
#   son absence est une situation prevue par la methode (codes 5/17).
# @param Dmax_optionnel    Profondeur maximale (m), NA si inconnue -- sert
#   uniquement a rejeter une profondeur moyenne incoherente (>= prof. max).
calc_boivin_vaillancourt_omble <- function(A, grp, tributaire_absent = NA,
                                           Dmn_optionnel = NA, Dmax_optionnel = NA) {
  if (is.na(A) || A <= 0) return(NULL)

  # --- 1. Classe d'especes : choisit la table (allopatrie ou meunier) --------
  classe <- classer_classe_boivin(grp)
  if (is.na(classe)) {
    non_couv <- setdiff(grp$brutes,
                        c(ESPECES_CATOSTOMES_OMBLE, ESPECES_CYPRINS_OMBLE, "allopatrie"))
    detail <- if (length(non_couv) > 0) paste0(" (", paste(non_couv, collapse = ", "), ")")
              else " (ménés sans meunier)"
    return(list(rendement_ha = NA_real_,
                note = paste0("Combinaison d'espèces non couverte par Vaillancourt et Boivin 2000",
                              detail, " — modèle indisponible.")))
  }

  # --- 2. Profondeur moyenne : connue, inconnue ou incoherente ----------------
  dmn_connue <- !is.na(Dmn_optionnel) && Dmn_optionnel > 0
  if (dmn_connue && !is.na(Dmax_optionnel) && Dmn_optionnel >= Dmax_optionnel) {
    # Une valeur incoherente n'est PAS traitee comme « inconnue » : cela
    # appliquerait silencieusement le code 5/17 a un lac mal saisi.
    return(list(rendement_ha = NA_real_, classe = classe,
                note = "Profondeur moyenne ≥ profondeur maximale — donnée à corriger, modèle indisponible."))
  }

  # --- 3. Premier code applicable, dans l'ordre du document -------------------
  situation <- if (isTRUE(tributaire_absent)) {
    "sans_tributaire"
  } else if (A < 2) {
    "sup_inf_2"
  } else if (A == 2) {
    NA_character_                      # lacune du document (ni < 2 ni > 2 ha)
  } else if (A <= 5) {
    "sup_2_5"
  } else if (!dmn_connue) {
    "sup_5_dmn_inconnue"
  } else if (Dmn_optionnel < 1.5) {
    "dmn_inf_1_5"
  } else if (Dmn_optionnel <= 2.5) {
    "dmn_1_5_2_5"
  } else {
    "dmn_connue"
  }
  ...
  if (is.na(situation)) {
    return(list(rendement_ha = NA_real_, classe = classe,
                note = "Superficie de 2 ha exactement : non couverte par le document (< 2 ha ou > 2 ha) — modèle indisponible."))
  }

  lig    <- TABLE_BOIVIN_2000[TABLE_BOIVIN_2000$situation == situation, ]
  code   <- lig[[paste0("code_", classe)]]
  valeur <- lig[[classe]]
  notes  <- character(0)

  if (lig$type == "non_considere") {
    return(list(rendement_ha = NA_real_, code = code, classe = classe,
                note = paste0("Code ", code, " (", lig$libelle, ") : rendement non considéré par la méthode.")))
  }

  base_vezina <- NA_real_
  if (lig$type == "pct_vezina") {
    # Dmn > 2,5 m ici : calc_vezina_omble() (bloquee sous 2 m) repond toujours.
    vz <- calc_vezina_omble(Dmn_optionnel)
    if (is.null(vz)) return(NULL)
    base_vezina <- vz$rendement_ha
    rendement   <- base_vezina * valeur
    notes <- c(notes, paste0(round(valeur * 100), " % du rendement Vézina (",
                             fmt_nb(base_vezina), " kg/ha)"))
    if (Dmn_optionnel > 25.9)
      notes <- c(notes, "profondeur moyenne > 25,9 m, au-delà du domaine retenu par Vaillancourt 1998")
  } else {
    rendement <- valeur
  }

  # Tributaire inconnu : les codes 1/13 n'ont pas pu etre evalues. Signale,
  # jamais suppose « present » en silence.
  if (is.na(tributaire_absent))
    notes <- c(notes, "tributaire/émissaire inconnu — code sans tributaire non évalué")

  list(rendement_ha = rendement,
       code         = code,
       classe       = classe,
       base_vezina  = if (is.na(base_vezina)) NA_real_ else round(base_vezina, 3),
       note = paste0("Code ", code, " (", lig$libelle, ", ",
                     if (classe == "meunier") "avec meunier" else "allopatrie", ")",
...
css_minimal <- "
  ...
  /* Trois cartes (recommandé | révisé | actuel), 2026-09 */
  .kpi-top-row.kpi-3 { grid-template-columns: repeat(3, minmax(0,1fr)); }
  .kpi-cellule { display: flex; min-width: 0; }
  .kpi-cellule > .kpi-top-card { width: 100%; padding: 20px 20px; }
  .kpi-3 .quota-hero-val { font-size: 34px; }
  .statut-enreg { font-size: 12px; color: #5a6b7b; }
  /* Barre d'export (2026-09 v2) */
  .export-barre { display: flex; justify-content: space-between; align-items: center; gap: 12px;
                  flex-wrap: wrap; background: white; border: 1px solid #dee2e6; border-radius: 8px;
                  padding: 10px 16px; }
  .export-compte { display: inline-block; min-width: 26px; padding: 2px 8px; margin-right: 8px;
                   border-radius: 12px; background: #2C3E50; color: white; font-weight: 700;
                   font-size: 13px; text-align: center; }
  .export-texte { font-size: 12.5px; color: #5a6b7b; }
  .btn-export { font-weight: 600; padding: 6px 16px; }
  .statut-enreg.perime { color: #A86E08; font-weight: 600; }
  @media (max-width: 1100px) { .kpi-top-row.kpi-3 { grid-template-columns: minmax(0,1fr); } }
  ...
  /* Figure « Positionnement sur une ligne » (kg/an, 2026-09) — sans couleur :
     les repères se distinguent par le style de trait et la forme du symbole */
  .lg-carte { background: white; border: 1px solid #dee2e6; border-radius: 8px;
              padding: 14px 18px; box-shadow: 0 1px 4px rgba(0,0,0,0.04); }
  .lg-zone  { position: relative; margin: 10px 28px 0; }
  .lg-haut, .lg-bas { position: relative; }
  .lg-etiq  { position: absolute; white-space: nowrap; font-size: 11.5px; line-height: 14px;
              color: #2C3E50; }
  /* Mise en forme uniforme (v4) : nom en gras, valeur et unité en normal */
  .lg-etiq .lg-nom { font-weight: 700; }
  .lg-etiq .lg-val { font-weight: 400; }
  .lg-lien  { position: absolute; width: 0; }
  .lg-barre { position: relative; height: 12px; background: #EEF0F2; border-radius: 3px; }
  .lg-trait { position: absolute; top: -5px; bottom: -5px; width: 0; }
  .lg-sym   { position: absolute; top: 50%; transform: translate(-50%,-50%); z-index: 3; }
  .lg-sym.ref  { width: 13px; height: 13px; border-radius: 50%; background: #2C3E50; }
  .lg-sym.mod  { width: 11px; height: 11px; border-radius: 50%; background: #FFFFFF;
                 border: 2px solid #5a6b7b; }
  .lg-sym.rec  { width: 0; height: 0; border-left: 7px solid transparent;
                 border-right: 7px solid transparent; border-bottom: 12px solid #2C3E50; }
  .lg-sym.rev  { width: 11px; height: 11px; background: #2C3E50; transform: translate(-50%,-50%) rotate(45deg); }
  .lg-axe   { position: relative; height: 20px; border-top: 1px solid #c3c2b7; margin-top: 8px; }
  .lg-tick  { position: absolute; top: 3px; transform: translateX(-50%); font-size: 11px;
              color: #898781; white-space: nowrap; }
  .lg-legende { display: flex; flex-wrap: wrap; gap: 14px; font-size: 11.5px; color: #5a6b7b; }
  .lg-leg-trait { display: inline-block; width: 22px; height: 0; vertical-align: middle; margin-right: 5px; }
  .lg-leg-sym { display: inline-block; vertical-align: middle; margin-right: 5px; }
...
bloc_donnees_lac_ui <- function() {
  div(class = "bloc-gauche",
    ...
    fileInput("pothal_file", NULL, accept = c(".xlsx", ".xlsm"),
              buttonLabel = "Importer le fichier de Potentiel halieutique",
              placeholder = ""),
    uiOutput("pothal_file_name"),

    # Fichier chargé : liste déroulante des lacs du fichier
    conditionalPanel("output.pothal_charge && !output.saisie_libre",
      uiOutput("lac_select_ui"),
      uiOutput("msg_lac"),
      # « Effacer la sélection » (2026-09) remplace « Effacer l'importation » :
      # le fichier reste chargé, on vide le lac choisi et on passe à la saisie
      # d'un lac absent de la liste.
      actionLink("btn_effacer_selection",
                 label = "\u2715 Effacer la sélection",
                 class = "lien-discret")
    ),
    # Aucun fichier, ou lac absent de la liste : identification manuelle
    # (nom obligatoire pour calculer)
    conditionalPanel("!output.pothal_charge || output.saisie_libre",
      div(class = "grille-nom",
        textInput("nom_lac", "Nom du lac"),
        textInput("no_lac",  "No lac")
      ),
      uiOutput("msg_nom"),
      conditionalPanel("output.pothal_charge",
        actionLink("btn_retour_liste", label = "\u21a9 Revenir à la liste des lacs",
                   class = "lien-discret"))
    ),
...
bloc_climat_ui <- function() {
  conditionalPanel(si_espece("touladi", "dore"),
    div(class = "bloc-gauche",
      ...
      # Coordonnées du point d'extraction (2026-09) : vides = coordonnées de
      # l'onglet Lacs ; saisies = elles priment (lac hors fichier, ou point
      # plus représentatif qu'un centroïde).
      div(class = "grille-2",
        champ_num("lat_manuelle", "Latitude (deg. déc.)",  placeholder = "ex. 48.6872",
                  step = 0.0001),
        champ_num("lon_manuelle", "Longitude (deg. déc.)", placeholder = "ex. -79.7094",
                  step = 0.0001)
      ),
      uiOutput("src_coord"),
      div(class = "grille-2 mt-2",
          ...
          conditionalPanel(si_espece("dore"),
            div(class = "mt-2",
              champ_num("degres_jours_g", "DJC5 (degrés-jours)",
                        placeholder = "ex. 1825", min = 0, src = TRUE)))
        )
      ),
      uiOutput("msg_climat")
    )
      ...
      conditionalPanel(si_espece("touladi"),
        div(class = "bloc-titre",
            info_tip("Espèces présentes",
                     paste0("Ajuste les rendements des modèles régionaux selon la communauté présente."))),
        
        checkboxGroupInput("especes_presentes_touladi", NULL,
          choices = c("Doré jaune"                    = "dore",
                      "Brochet"                       = "brochet",
                      "Achigan"                       = "achigan",
                      "Omble de fontaine (abondant)" = "omble_dom"),
          selected = NULL)
      ),
      conditionalPanel(si_espece("dore"),
        div(class = "bloc-titre",
...
LIBELLES_INTRANTS <- c(
  ...
  Dmn_optionnel = "Prof. moyenne (facultative)", Dmax_optionnel = "Prof. max (contrôle, facultative)"
)
...
page_information_ui <- function() {
  ...
  bloc_espece <- function(cle) {
    ...
    lignes <- lapply(casc, function(k) {
      m    <- esp$modeles[[k]]
      role <- if (isTRUE(m$quota_depart)) "Comparaison — quota de départ (hors cascade)"
              else if (k == casc[1]) "Recommandé"
              else paste0("Repli n\u00b0 ", which(casc == k) - 1)
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$script(HTML("
      // Cases « Comparer » du tableau des modèles -> figure (2026-09)
      $(document).on('change', 'input.cmp-mod', function() {
    ...
    column(8, style = "padding: 20px;",
      ...
      tabsetPanel(id = "tabs_resultats",
        ...
        tabPanel("Rendement théorique",
          ...
          # Zone 1 — Décision (kg/an), trois cartes (2026-09) :
          #   Quota recommandé (80 %, non modifiable) | Quota révisé (taux
          #   modifiable + « Enregistrer ce quota ») | Quota actuel.
          # La carte « révisé » est statique : son champ de taux n'est jamais
          # recréé par un renderUI (évite la perte du curseur pendant la saisie).
          div(class = "kpi-top-row kpi-3",
            uiOutput("carte_reco_ui", class = "kpi-cellule"),
            div(class = "kpi-cellule",
              div(class = "kpi-top-card carte-revise",
                ...
                div(class = "quota-derivation",
                  numericInput("pct_manual", label = NULL, value = PCT_RECOMMANDE,
                               min = 1, max = 200, step = 1, width = "72px"),
                  span(class = "text-muted", "%"),
                  uiOutput("revise_derivation_ui", inline = TRUE)),
                uiOutput("revise_ecart_ui"),
                div(class = "d-flex align-items-center gap-2 flex-wrap mt-2",
                  actionButton("btn_enregistrer_quota", "Enregistrer ce quota",
                               icon = icon("floppy-disk"),
                               class = "btn btn-sm btn-outline-primary"),
                  uiOutput("revise_statut_ui", inline = TRUE))
              )
            ),
            uiOutput("carte_actuel_ui", class = "kpi-cellule")
          ),

          # Zone 2 — Comparaison (kg/an) : une seule ligne, modèles à cocher
          ...
          # Provenance des intrants du modèle de référence (déplacée sous le
          # tableau, 2026-09 — conservée : seul endroit où les valeurs
          # théoriques utilisées par le quota sont signalées)
          uiOutput("intrants_ui"),

          # Export (2026-09 v2) : un seul bouton, Excel des quotas enregistrés
          # (le bouton PDF, jamais implémenté, est retiré)
          div(class = "export-barre mt-3",
            uiOutput("export_hint_ui"),
            downloadButton("dl_excel", "Exporter les quotas enregistrés (Excel)",
...
server <- function(input, output, session) {
  ...
  pothal_brut <- reactive({
    ...
    tryCatch({
      ...
      list(
        ...
        lacs_brutes = lacs,
        profil      = profil,
        parametre   = pivoter_parametre(parametre, profil),
        specimens    = specimens,
  ...
  ifa_habitat_raw <- reactive({
    ...
    tryCatch({
      ...
      # --- Conductivité : Parametre (code CD) et Profil, voir plus bas -------
      ...
      # --- pH : Parametre (code PH) ET Profil (lecture la plus proche de 5 m)
      # — la valeur la plus récente des deux est retenue ; à date égale, le
      # Profil l'emporte (règle confirmée 2026-09). Combinaison faite plus bas,
      # une fois les lignes Profil du lac extraites.
      ...
      # pH et conductivité (règle confirmée 2026-09 v2) : la mesure la plus
      # RÉCENTE entre Parametre et Profil (lecture la plus proche de 5 m, station
      # la plus profonde) ; le Profil l'emporte à date égale ou si la date
      # Parametre est inconnue. Valeurs propres au LAC, pas à l'inventaire.
      date_param_brute <- function(code) {
        pp <- pb$parametre
        if (is.null(pp) || !("date" %in% names(pp))) return(as.Date(NA))
        l <- pp[normaliser_nolac(pp$nolac) == normaliser_nolac(nolac_val) & pp$code_param == code, ]
        if (nrow(l) == 0) as.Date(NA) else as.Date(l$date[1])
      }
      combiner_profil_param <- function(col, code, val_param, date_txt_param) {
        pr <- extraire_profil_5m(df_lac, col)
        d_par <- date_param_brute(code)
        prendre_profil <- !is.na(pr$valeur) &&
          (is.na(val_param) || is.na(d_par) || (!is.na(pr$date) && pr$date >= d_par))
        if (prendre_profil)
          list(val = pr$valeur, source = "Profil", date = texte_profil_5m(pr))
        else
          list(val = val_param, source = if (!is.na(val_param)) "Parametre" else NA_character_,
               date = date_txt_param)
      }
      ph_c   <- combiner_profil_param("ph",   "PH", ph_parametre,   ph_date_param)
      cond_c <- combiner_profil_param("cond", "CD", cond_parametre, cond_date_param)
      ph_val <- ph_c$val; ph_source <- ph_c$source; ph_date <- ph_c$date
      ...
      # Profil thermique et d'oxygène : UNE seule station par inventaire
      # (règle confirmée 2026-09 v2). Avant : les lectures de toutes les
      # stations d'un inventaire étaient moyennées profondeur par profondeur,
      # ce qui mélangeait des bassins et pouvait déplacer la thermocline. On
      # garde la station la plus profonde parmi celles qui ont des lectures de
      # température (égalité : le plus grand nombre de lectures).
      st_par_inv <- lapply(split(df_lac, df_lac$inv_key), function(sub)
        station_la_plus_profonde(sub[!is.na(sub$temp) & !is.na(sub$prof_mes), ]))
      df_lac <- dplyr::bind_rows(lapply(names(st_par_inv), function(k) {
        sub <- df_lac[df_lac$inv_key == k, ]
        st  <- st_par_inv[[k]]
        if (is.na(st$station)) sub
        else sub[!is.na(sub$station) & cle_station(sub$station) == st$station, ]
      }))
      ...
      invs <- do.call(rbind, lapply(unique(df_lac$inv_key), function(k) {
        ...
        data.frame(
          ...
          cond          = cond_c$val,
          cond_source   = cond_c$source,
          cond_date     = cond_c$date,
          secchi        = secchi_parametre,
          secchi_source = if (!is.na(secchi_parametre)) "Parametre" else NA_character_,
          secchi_date   = secchi_date_param,
          ph            = ph_val,
  ...
  observeEvent(input$ifa_inv_choisi, {
    ...
    # (2026-09 v2) La conductivité est maintenant propre au lac (plus récente
    # entre Parametre et Profil) : plus rien à réécrire au changement
    # d'inventaire. Observateur conservé vide pour documenter le choix.
  }, ignoreInit = TRUE)
  ...
  # Coordonnées saisies (les DEUX champs requis) : priment sur le fichier.
  # Debounce : l'extraction relit les rasters, inutile de la lancer à chaque
  # chiffre tapé.
  coord_saisie <- debounce(reactive({
    la <- input$lat_manuelle; lo <- input$lon_manuelle
    if (is.null(la) || is.null(lo) || is.na(la) || is.na(lo)) c(lat = NA_real_, lon = NA_real_)
    else c(lat = la, lon = lo)
  }), 800)

  coord_lac <- reactive({
    cs <- coord_saisie()
    if (!is.na(cs[["lat"]]) && !is.na(cs[["lon"]])) return(cs)
    coord_fichier()
  })

  output$src_coord <- renderUI({
    la <- input$lat_manuelle; lo <- input$lon_manuelle
    cf <- coord_fichier()
    n_saisis <- sum(!is.na(c(la %||% NA, lo %||% NA)))
    fich_txt <- if (!is.na(cf[["lat"]]) && !is.na(cf[["lon"]]))
      paste0(fmt_nb(cf[["lat"]], 4), ", ", fmt_nb(cf[["lon"]], 4)) else NULL
    # Au Québec, la longitude est négative (hémisphère ouest) : une valeur
    # positive est presque toujours un signe « − » oublié. Vérification de
    # signe seulement — l'appartenance à la grille est contrôlée à l'extraction.
    alerte_signe <- if (!is.null(lo) && !is.na(lo) && lo > 0)
      msg_ui("warning", "Longitude positive — au Québec, la longitude est négative (ouest).")
    tagList(
      if (n_saisis == 2)
        div(class = "src-ligne", paste0("Coordonnées saisies",
            if (!is.null(fich_txt)) paste0(" (onglet Lacs : ", fich_txt, ")") else ""))
      else if (n_saisis == 1)
        msg_ui("warning", "Saisir la latitude ET la longitude pour les utiliser.")
      else if (!is.null(fich_txt))
        div(class = "src-ligne", paste0("Onglet Lacs : ", fich_txt))
      else
        div(class = "src-ligne", "Aucune coordonnée — saisir pour extraire le climat."),
      alerte_signe)
  })
  ...
  output$msg_lac <- renderUI({
    ...
    msg_ui("info", paste0("Plusieurs UE trouvées (", paste(unique(suff), collapse = ", "),
                          ") — ", retenu, " en priorité, repli sur les autres UE ",
                          "pour les valeurs absentes."))
  })

  # Provenance de la morphométrie (onglet Lacs) : UE principale du lac et,
  # le cas échéant, variables reprises d'une autre UE parce qu'absentes de la
  # principale (repli IPE -> OG -> autre, voir dedup_lacs_par_ue()).
  src_morpho_ui <- function() {
    pb        <- tryCatch(pothal_brut(), error = function(e) NULL)
    nolac_val <- cle_lac(lac_courant_rv())
    if (is.null(pb) || is.na(nolac_val) || !isTRUE(pothal_actif_rv())) return(NULL)
    lg <- pb$lacs[!is.na(pb$lacs$nolac) & pb$lacs$nolac == nolac_val, ]
    if (nrow(lg) == 0) return(NULL)
    ue_princ <- as.character(lg$ue_min[1])
    suff     <- if (!is.na(ue_princ)) toupper(trimws(sub(".*-", "", ue_princ))) else NA_character_
    libs <- c(sup = "superficie", prof_max = "prof. max", prof_moy = "prof. moy.")
    champs <- list(sup = input$sup, prof_max = input$prof_max, prof_moy = input$prof_moy)
    details <- unlist(lapply(names(libs), function(v) {
      col_ue <- paste0(v, "_ue")
      val_f  <- if (v %in% names(lg)) lg[[v]][1] else NA_real_
      ch     <- champs[[v]]
      modif  <- !is.na(val_f) && !is.null(ch) && !is.na(ch) && abs(ch - val_f) > 1e-6
      ue_v   <- if (col_ue %in% names(lg)) as.character(lg[[col_ue]][1]) else NA_character_
      repli  <- !is.na(val_f) && !is.na(ue_v) && !identical(ue_v, ue_princ)
      c(if (repli) paste0(libs[[v]], " : repli ", ue_v,
                          if (!is.na(suff)) paste0(" (absente de ", suff, ")") else ""),
        if (modif) paste0(libs[[v]], " modifiée (fichier : ", fmt_nb(val_f, 1), ")"))
    }))
    div(class = "src-ligne",
        paste(c(paste0("Onglet Lacs", if (!is.na(ue_princ)) paste0(", UE ", ue_princ) else ""),
                details), collapse = " \u00b7 "))
  }
  ...
  ligne_param <- function(champ, fichier, source, date, dec, suffixe = NULL) {
    ...
    else if (identical(source, "Profil"))
      paste0("Onglet Profil, ", date)
  ...
  results <- eventReactive(input$btn_calc, {
    ...
    bassin <- list(
      ...
      A_optionnel        = sup,
      # Profondeurs dupliquees sous un autre nom pour Vaillancourt-Boivin 2000 :
  ...
  # ---------------------------------------------------------------------------
  # Mode « lac absent de la liste » : le fichier reste chargé, mais le lac est
  # identifié par les champs Nom / No et toutes les données sont saisies.
  saisie_libre_rv <- reactiveVal(FALSE)
  output$saisie_libre <- reactive(isTRUE(saisie_libre_rv()))
  outputOptions(output, "saisie_libre", suspendWhenHidden = FALSE)

  observeEvent(input$btn_effacer_selection, {
    reinitialiser_lac()
    updateSelectInput(session, "lac_select", selected = "")
    saisie_libre_rv(TRUE)
  })
  observeEvent(input$btn_retour_liste, {
    reinitialiser_lac()
    saisie_libre_rv(FALSE)
  })
  # Nouvel import : retour à la liste
  observeEvent(input$pothal_file, saisie_libre_rv(FALSE), ignoreInit = TRUE)
  ...
  # ZONE 1 — Décision, en kg/an, trois cartes (révision 2026-09) :
  #   - Quota recommandé : TOUJOURS au taux recommandé (80 %), non modifiable ;
  #   - Quota révisé     : taux saisi par le biologiste, enregistrable ;
  #   - Quota actuel     : quota en vigueur, comparé aux deux autres.
  # Toutes les valeurs dérivent du même produit :
  #   quota = taux × rendement maximal théorique (kg/ha, modèle de référence)
  #           × superficie (ha)
  # Le taux est un terme de l'équation, pas un réglage d'affichage.
  quota_base <- reactive({
    calc_fait <- isTRUE(calc_valide_rv()) &&
                 !is.null(tryCatch(results(), error = function(e) NULL))
    out <- list(calc_fait = calc_fait, ref_m = NULL, max_ha = NA_real_, sup = NA_real_,
                max_an = NA_real_, q_reco = NA_real_, q_rev = NA_real_, pct = pct_rv())
    if (!calc_fait) return(out)
    r <- results()
    out$ref_m <- modele_reference(r, config()$cascade_reference, config()$modeles)
    out$sup   <- r$sup
    if (!is.null(out$ref_m) && !is.na(out$ref_m$val) && !is.na(out$sup) && out$sup > 0) {
      out$max_ha <- out$ref_m$val
      out$max_an <- out$max_ha * out$sup
      out$q_reco <- round(out$max_an * PCT_RECOMMANDE / 100)
      out$q_rev  <- round(out$max_an * out$pct / 100)
    }
    out
  })

  # Ligne « X % de Y kg/ha × Z ha »
  derivation_txt <- function(qb) {
    if (!qb$calc_fait || is.na(qb$max_ha)) return(span(class = "text-muted", " du maximum théorique"))
    tagList(span(paste0(" de ", fmt_nb(qb$max_ha), " kg/ha")),
            span(class = "quota-derivation-op", "\u00d7"),
            span(paste0(fmt_nb(qb$sup, 0), " ha")))
  }

  # Écart signé entre deux quotas, en texte (« 1 166 kg/an au-dessus du … »)
  ecart_txt <- function(a, b, lib_b) {
    if (is.na(a) || is.na(b)) return(NULL)
    e <- a - b
    if (abs(e) < 1) return(paste0("Équivalent au ", lib_b))
    paste0(fmt_int(abs(e)), " kg/an ", if (e > 0) "au-dessus du " else "sous le ", lib_b)
  }

  # --- Carte 1 : quota recommandé (fixe) ------------------------------------
  output$carte_reco_ui <- renderUI({
    qb <- quota_base()
    tip <- paste0("Quota recommandé = ", PCT_RECOMMANDE, " % × rendement maximal théorique du ",
                  "modèle de référence × superficie. Le taux de ", PCT_RECOMMANDE, " % suit le ",
                  "concept de « pretty good yield » (Hilborn 2010) : pêcher à ~80 % du maximum ",
                  "conserve l'essentiel du rendement en réduisant nettement le risque de ",
                  "surexploitation.")
    div(class = "kpi-top-card zone-recommande",
      div(class = "kpi-eyebrow", info_tip("Quota recommandé", tip)),
      div(span(class = "quota-hero-val", if (is.na(qb$q_reco)) "\u2014" else fmt_int(qb$q_reco)),
          span(class = "quota-hero-unit", "kg/an")),
      div(class = "quota-derivation",
          span(tags$strong(paste0(PCT_RECOMMANDE, " %"))), derivation_txt(qb)),
      if (qb$calc_fait && !is.null(qb$ref_m)) {
        k_reco <- config()$cascade_reference[1]
        if (identical(qb$ref_m$cle, k_reco))
          tags$small(class = "kpi-sub mt-1 d-block", paste0("Modèle recommandé : ", qb$ref_m$nom))
        else
          tags$small(class = "kpi-sub mt-1 d-block fw-bold",
                     paste0("\u26a0 Modèle recommandé (", config()$modeles[[k_reco]]$nom,
  ...
  # --- Carte 2 : quota révisé -------------------------------------------------
  output$revise_valeur_ui <- renderUI({
    qb <- quota_base()
    div(span(class = "quota-hero-val", if (is.na(qb$q_rev)) "\u2014" else fmt_int(qb$q_rev)),
        span(class = "quota-hero-unit", "kg/an"))
  })
  output$revise_derivation_ui <- renderUI(derivation_txt(quota_base()))
  output$revise_ecart_ui <- renderUI({
    qb <- quota_base()
    if (is.na(qb$q_rev)) return(NULL)
    tagList(
      if (!isTRUE(qb$pct == PCT_RECOMMANDE))
        tags$small(class = "kpi-sub d-block", ecart_txt(qb$q_rev, qb$q_reco, "quota recommandé")),
      if (qb$pct > 100)
        tags$small(class = "kpi-sub d-block fw-bold",
                   "\u26a0 Au-delà du maximum théorique du modèle de référence"))
  })

  # --- Carte 3 : quota actuel -------------------------------------------------
  output$carte_actuel_ui <- renderUI({
    qb <- quota_base()
    q  <- quota_actuel_val()
    if (is.na(q)) {
      return(div(class = "kpi-top-card",
        div(class = "kpi-eyebrow", "Quota actuel"),
        div(span(class = "quota-hero-val", style = "color:#adb5bd;", "\u2014"),
            span(class = "quota-hero-unit", "kg/an")),
        tags$small(class = "kpi-sub mt-2 d-block", "Non saisi (bloc « Données d'exploitation »)")))
    }
    pct_act <- if (!is.na(qb$max_an) && qb$max_an > 0) q / qb$max_an * 100 else NA_real_
    div(class = "kpi-top-card",
      div(class = "kpi-eyebrow", "Quota actuel"),
      div(span(class = "quota-hero-val", fmt_int(q)), span(class = "quota-hero-unit", "kg/an")),
      if (!qb$calc_fait) tags$small(class = "kpi-sub mt-2 d-block", "Calculer pour comparer")
      else tagList(
        tags$small(class = "kpi-sub mt-2 d-block fw-bold", ecart_txt(q, qb$q_reco, "quota recommandé")),
        if (!isTRUE(qb$pct == PCT_RECOMMANDE))
          tags$small(class = "kpi-sub d-block", ecart_txt(q, qb$q_rev, "quota révisé")),
        if (!is.na(pct_act))
  ...
  # ---------------------------------------------------------------------------
  # QUOTAS ENREGISTRÉS — session en cours (2026-09)
  #
  #   Une ligne par (espèce, lac) ; réenregistrer le même lac pour la même
  #   espèce REMPLACE la ligne. Conservé en mémoire seulement : fermer ou
  #   recharger la page (ou la mise en veille du serveur) efface la table —
  #   exporter avant de quitter.
  #
  #   Masse moyenne prélevée et Succès : moyennes annuelles sur les 5
  #   DERNIÈRES ANNÉES DISPONIBLES du lac (règle fixe pour l'export, 2026-09
  #   v2 — indépendante de la fenêtre affichée sous la figure). La fenêtre
  #   part de la dernière saison présente dans les données ; les années sans
  #   donnée à l'intérieur de la fenêtre ne sont pas comblées. La colonne
  #   « Période exploitation » indique les années réellement couvertes.
  #   Le succès est la MOYENNE DES SUCCÈS ANNUELS (chaque année pèse autant),
  #   comme dans l'onglet d'analyse — et non le ratio captures totales /
  #   effort total, qui pondérerait par l'effort. Renseignés uniquement si le
  #   lac d'exploitation est le lac du calcul.
  # ---------------------------------------------------------------------------
  quotas_enregistres_rv <- reactiveVal(data.frame())

  cle_enregistrement <- function() {
    no <- cle_lac_potentiel()
    id <- if (!is.na(no)) no else trimws(input$nom_lac %||% "")
    paste0(espece_active_rv(), "|", id)
  }

  observeEvent(input$btn_enregistrer_quota, {
    qb <- quota_base()
    if (!qb$calc_fait || is.na(qb$q_rev)) {
      showNotification("Calculer le quota avant de l'enregistrer.", type = "warning", duration = 4)
      return()
    }
    # Données d'exploitation : même lac seulement (voir correspondance_lacs),
    # toujours sur les 5 dernières années disponibles
    corr   <- correspondance_lacs()
    df_obs <- if (corr %in% c("identique", "non_verifiable")) tryCatch({
                base <- exploit_edited_rv()
                if (is.null(base) || nrow(base) == 0 || is.na(sup_exploit())) NULL
                else appliquer_fenetre(ajouter_indicateurs(base, sup_exploit()), "5")
              }, error = function(e) NULL) else NULL
    a_obs  <- !is.null(df_obs) && nrow(df_obs) > 0
    moy    <- function(x) { m <- mean(x, na.rm = TRUE); if (is.finite(m)) m else NA_real_ }
    note_obs <- switch(corr,
      aucun          = "Aucune donnée d'exploitation sélectionnée",
      different      = "Lac d'exploitation différent — non reporté",
      non_verifiable = "Correspondance des lacs non vérifiable (no lac absent)",
      if (!a_obs) "Aucune donnée d'exploitation pour ce lac" else "")
    periode <- if (a_obs) {
      an <- sort(unique(df_obs$annee))
      paste0(min(an), "\u2013", max(an), " (", length(an), " an", if (length(an) > 1) "s",
             " de données sur les 5 dernières années disponibles)")
    } else NA_character_

    no <- cle_lac_potentiel()
    ligne <- data.frame(
      cle                             = cle_enregistrement(),
      .taux                           = qb$pct,   # interne : restaure le taux ; non exporté
      "Espèce"                        = config()$nom,
      ...
      "Succès moy. (n/j-p)"           = if (a_obs) round(moy(df_obs$succes), 3) else NA_real_,
      "Période exploitation"          = periode,
      "Note exploitation"             = note_obs,
      "Enregistré le"                 = format(Sys.time(), "%Y-%m-%d %H:%M"),
  ...
  # Statut sous le bouton : enregistré ? toujours à jour ?
  output$revise_statut_ui <- renderUI({
    tab <- quotas_enregistres_rv()
    if (nrow(tab) == 0) return(NULL)
    l <- tab[tab$cle == cle_enregistrement(), , drop = FALSE]
    if (nrow(l) == 0) return(NULL)
    qb <- quota_base()
    a_jour <- !is.na(qb$q_rev) && isTRUE(l[["Quota révisé (kg/an)"]][1] == qb$q_rev)
    span(class = paste("statut-enreg", if (!a_jour) "perime"),
         if (a_jour) paste0("\u2713 Enregistré (", l[["Enregistré le"]][1], ")")
         else paste0("Modifié depuis l'enregistrement (", fmt_int(l[["Quota révisé (kg/an)"]][1]),
                     " kg/an) — réenregistrer"))
  })

  # Changement de lac ou d'espèce : le taux révisé repart du taux enregistré
  # pour ce lac et cette espèce, sinon du taux recommandé.
  # Fenêtre de la récolte observée : 5 ans par défaut à chaque changement de lac
  observeEvent(lac_courant_rv(), fenetre_rv("5"), ignoreInit = TRUE)

  observeEvent(list(lac_courant_rv(), espece_active_rv(), saisie_libre_rv()), {
    tab <- quotas_enregistres_rv()
    l   <- if (nrow(tab) > 0) tab[tab$cle == cle_enregistrement(), , drop = FALSE] else tab
    p   <- if (nrow(l) > 0) l[[".taux"]][1] else PCT_RECOMMANDE
    pct_rv(p)
    updateNumericInput(session, "pct_manual", value = p)
  }, ignoreInit = TRUE)
  ...
  source_param <- function(lg, cle, champ) {
    ...
    if (identical(src, "Profil (repli)") || identical(src, "Profil")) paste0("profil ", dt)
  ...
  # ---------------------------------------------------------------------------
  # FIGURE — Positionnement sur une ligne (kg/an)   [révision 2026-09]
  #
  #   Retour à une figure à UNE seule ligne (demande du 2026-09), mais :
  #     - axe partant de ZÉRO et en kg/an (l'unité de la décision, mêmes
  #       chiffres que les cartes) — l'ancienne barre en kg/ha « zoomait »
  ...
  #   chevauchements.
  #   Rendement observé = masse récoltée annuelle moyenne (kg/an) sur la
  #   fenêtre choisie, affiché seulement si le lac d'exploitation est celui du
  #   calcul (sinon il porterait sur un autre lac).
  # ---------------------------------------------------------------------------
  modeles_compares_rv <- reactiveVal(character(0))
  observeEvent(input$modele_comparer, {
    ev  <- input$modele_comparer
    cur <- modeles_compares_rv()
    modeles_compares_rv(if (isTRUE(ev$coche)) union(cur, ev$cle) else setdiff(cur, ev$cle))
  })
  # Les clés diffèrent d'une espèce à l'autre : on repart de zéro
  observeEvent(espece_active_rv(), modeles_compares_rv(character(0)), ignoreInit = TRUE)

  #' Répartit des étiquettes en couloirs sans chevauchement.
  #' Largeur estimée en % de la piste (~0,75 % par caractère à ~11,5 px sur
  #' une piste de ~850 px) — approximation d'affichage seulement.
  #' @return vecteur d'indices de couloir (0 = le plus proche de la ligne)
  couloirs_etiquettes <- function(x_pct, textes, n_max = 4L) {
    n <- length(x_pct); if (n == 0) return(integer(0))
    larg <- nchar(textes) * 0.75 + 1.5
    occ  <- vector("list", n_max)
    res  <- integer(n)
    for (i in order(x_pct)) {
      g <- x_pct[i] - larg[i] / 2; d <- x_pct[i] + larg[i] / 2
      k <- 1L
      while (k <= n_max && any(vapply(occ[[k]], function(iv) !(d < iv[1] || g > iv[2]), logical(1))))
        k <- k + 1L
      if (k > n_max) k <- n_max   # dernier couloir : chevauchement accepté
      occ[[k]] <- c(occ[[k]], list(c(g, d)))
      res[i] <- k - 1L
    }
    res
  }
  ...
  output$gauge_rdr_ui <- renderUI({
    qb <- quota_base()
    if (!qb$calc_fait || is.na(qb$max_an) || qb$max_an <= 0) return(NULL)
    r   <- results()
    sup <- qb$sup
    ref_m <- qb$ref_m

    q_reco <- qb$q_reco
    q_rev  <- if (!isTRUE(qb$pct == PCT_RECOMMANDE)) qb$q_rev else NA_real_
    ...
    # Modèles affichés : UNIQUEMENT ceux cochés dans le tableau (2026-09 v2 —
    # le maximum de référence n'est plus affiché d'office). Le modèle utilisé
    # en référence garde un symbole plein ; les autres, un symbole vide.
    coches <- modeles_compares_rv()
    pts <- list()
    for (k in config()$cascade_reference) {
      if (!(paste0("mod:", k) %in% coches)) next
      res <- r[[k]]
      if (!is.null(res) && !is.na(res$rendement_ha)) {
        # Aucun suffixe « (recommandé) » (2026-09 v3) : il se confondait avec le
        # quota recommandé. Seul le symbole (plein = référence) les distingue.
        est_ref <- identical(k, ref_m$cle)
        pts[[length(pts) + 1]] <- list(nom = config()$modeles[[k]]$nom,
                                       val = res$rendement_ha * sup,
                                       type = if (est_ref) "ref" else "mod")
      }
    }
    for (g in regions_actives()) {
      if (!(paste0("reg:", g$key) %in% coches) || is.na(g$val)) next
      pts[[length(pts) + 1]] <- list(nom = g$nom, val = as.numeric(g$val) * sup, type = "mod")
    }

    # Échelle : 0 -> plus grande valeur affichée + 8 %, graduations « propres »
    tous  <- c(vapply(pts, `[[`, numeric(1), "val"), q_reco, q_rev, q_act, q_obs)
    kg    <- function(v) paste0(fmt_int(v), " kg/an")
    ticks <- pretty(c(0, max(tous, na.rm = TRUE) * 1.08), n = 6)
    xmax  <- max(ticks)
    xp    <- function(v) 100 * v / xmax

    # --- Éléments au-dessus : repères de quota + modèles ---------------------
    haut <- Filter(Negate(is.null), c(
      # Étiquettes sur deux lignes (2026-09 v3) : nom puis valeur — environ deux
      # fois moins larges, donc moins d'empilement quand on coche des modèles.
      # Aucun pourcentage du maximum (retirés, v3) ; le taux reste seulement
      # pour le quota révisé, puisqu'il le définit.
      list(if (!is.na(q_reco)) list(v = q_reco, nom = "Quota recommandé", val = kg(q_reco),
                                   trait = "3px solid #2C3E50", cls = "", sym = NULL)),
      list(if (!is.na(q_rev)) list(v = q_rev,
                                  nom = paste0("Quota révisé (", qb$pct, " %)"), val = kg(q_rev),
                                  trait = "1.5px solid #2C3E50", cls = "", sym = "rev")),
      lapply(pts, function(p) list(v = p$val, nom = p$nom, val = kg(p$val),
                                   trait = "1px solid #c3c2b7",
                                   cls = "", sym = p$type))
    ))
    # --- En dessous : quota actuel (trait = limite) et récolte observée
    # (triangle = mesure) — formes différentes pour ne pas confondre une
    # limite réglementaire avec ce qui a été prélevé.
    bas <- Filter(Negate(is.null), list(
      if (!is.na(q_act)) list(v = q_act, nom = "Quota actuel", val = kg(q_act),
                              trait = "2px dashed #2C3E50", cls = "", sym = NULL),
      if (!is.na(q_obs)) list(v = q_obs,
                              nom = "Récolte observée", val = kg(q_obs),
                              trait = "1px solid #898781", cls = "", sym = "rec")
    ))

    H <- 32L  # hauteur d'un couloir (px) — deux lignes de texte
    placer <- function(el, dessus) {
      if (length(el) == 0) return(list(ui = NULL, h = 0L))
      x  <- vapply(el, function(e) xp(e$v), numeric(1))
      # Largeur d'une étiquette = sa plus longue ligne (nom ou valeur)
      cl <- couloirs_etiquettes(x, vapply(el, function(e)
              if (nchar(e$nom) >= nchar(e$val)) e$nom else e$val, character(1)))
      h  <- (max(cl) + 1L) * H + 4L
      ui <- lapply(seq_along(el), function(i) {
        e <- el[[i]]
        # Ancrage : centré, sauf près des bords (aligné pour rester dans la carte)
        ancre <- if (x[i] < 8) "transform:translateX(-4px);"
                 else if (x[i] > 92) "transform:translateX(calc(-100% + 4px));"
                 else "transform:translateX(-50%);"
        pos_v <- paste0(if (dessus) "bottom:" else "top:", 4L + cl[i] * H, "px;")
        lien  <- paste0(if (dessus) "bottom:0;" else "top:0;", "height:", 4L + cl[i] * H, "px;")
        tagList(
          div(class = paste("lg-etiq", e$cls),
              style = paste0("left:", round(x[i], 2), "%;", pos_v, ancre,
                             if (x[i] > 92) "text-align:right;" else if (x[i] >= 8) "text-align:center;" else ""),
              div(class = "lg-nom", e$nom), div(class = "lg-val", e$val)),
          div(class = "lg-lien",
              style = paste0("left:", round(x[i], 2), "%;", lien, "border-left:", e$trait, ";")))
      })
      list(ui = ui, h = h)
    }
    ph <- placer(haut, TRUE)
    ...
    # Traits et symboles sur la ligne
    sur_barre <- lapply(c(haut, bas), function(e) tagList(
      if (is.null(e$sym) || identical(e$sym, "rev"))
        div(class = "lg-trait", style = paste0("left:", round(xp(e$v), 2), "%; border-left:", e$trait, ";")),
      if (!is.null(e$sym))
        div(class = paste("lg-sym", e$sym), style = paste0("left:", round(xp(e$v), 2), "%;"),
            title = paste(e$nom, e$val))))

    axe <- div(class = "lg-axe",
      lapply(seq_along(ticks), function(i) div(class = "lg-tick",
        style = paste0("left:", round(xp(ticks[i]), 2), "%;",
                       if (i == 1) "transform:none;" else if (i == length(ticks)) "transform:translateX(-100%);" else ""),
        fmt_int(ticks[i]))))

    # --- Pied : fenêtre du rendement observé + légende -------------------------
    sel_fen  <- fenetre_rv()
    ...
    # Période de la récolte observée : sous le sélecteur 5/10/20 ans (v3),
    # plus sur la ligne. Années réellement présentes dans la fenêtre.
    an_obs <- if (!is.na(q_obs)) sort(unique(df_obs$annee)) else integer(0)
    ligne_obs <- div(
      div(class = "d-flex align-items-center gap-2 flex-wrap",
        tags$small(class = "kpi-sub",
          if (!a_recolte) "Récolte observée : aucune donnée d'exploitation pour ce lac"
          else if (!meme_lac) "Récolte observée : lac d'exploitation différent — non affichée"
          else if (is.na(q_obs)) "Récolte observée : aucune masse exploitable"
          else "Récolte observée (moyenne des dernières années disponibles) :"),
        toggle_n),
      if (length(an_obs) > 0)
        tags$small(class = "kpi-sub d-block",
                   paste0("Années couvertes : ", min(an_obs), "\u2013", max(an_obs), " (",
                          length(an_obs), " an", if (length(an_obs) > 1) "s", " de données)")))

    leg_trait <- function(style, txt) span(span(class = "lg-leg-trait", style = paste0("border-top:", style, ";")), txt)
    legende <- div(class = "lg-legende",
      leg_trait("3px solid #2C3E50", paste0("Recommandé (", PCT_RECOMMANDE, " %)")),
      span(span(class = "lg-leg-sym", style = "width:9px;height:9px;background:#2C3E50;transform:rotate(45deg);"), "Révisé"),
      leg_trait("2px dashed #2C3E50", "Quota actuel"),
      span(span(class = "lg-leg-sym", style = "width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-bottom:10px solid #2C3E50;"), "Récolte observée"),
      span(span(class = "lg-leg-sym", style = "width:11px;height:11px;border-radius:50%;background:#2C3E50;"), "Modèle de référence"),
      span(span(class = "lg-leg-sym", style = "width:11px;height:11px;border-radius:50%;border:2px solid #5a6b7b;"), "Modèle comparé"))

    div(class = "mb-3 lg-carte",
      tags$strong(style = "font-size:12.5px;", "Positionnement des quotas (kg/an)"),
      div(class = "lg-zone",
        div(class = "lg-haut", style = paste0("height:", ph$h, "px;"), ph$ui),
        div(class = "lg-barre", sur_barre),
        div(class = "lg-bas", style = paste0("height:", pb$h, "px;"), pb$ui),
        axe),
  ...
  output$table_modeles <- DT::renderDT({
    ...
    # Colonne « Comparer » rétablie (2026-09) : la figure revient à une seule
    # ligne ; cocher un modèle l'y ajoute. L'état des cases est lu avec
    # isolate() : cocher ne redessine pas le tableau (la case garde son état
    # dans le navigateur), il ne met à jour que la figure.
    coches_tab <- isolate(modeles_compares_rv())
    # (2026-09 v2) Toutes les lignes sont cochables, y compris le modèle de
    # référence, qui n'est plus affiché d'office sur la figure.
    case_cmp <- function(cle) {
      paste0("<input type='checkbox' class='cmp-mod' data-cle='", cle, "'",
             if (cle %in% coches_tab) " checked" else "", ">")
    }
    ...
    ref_key  <- keys_mod[1]   # modèle RECOMMANDÉ (1er de la cascade)
    # Modèle réellement utilisé pour le quota : le 1er disponible de la cascade.
    # Diffère du recommandé quand une donnée obligatoire manque (2026-09 v2).
    ref_utilise <- if (calc_fait) {
      m <- modele_reference(tryCatch(results(), error = function(e) NULL), keys_mod, config()$modeles)
      if (is.null(m)) NA_character_ else m$cle
    } else NA_character_
    lib_mod  <- list(
      ...
      omble_boivin  = "Vaillancourt et Boivin 2000"
    )
    label_mod <- function(k) {
      lab <- lib_mod[[k]]
      # (2026-09 v2) Le 1er modèle garde TOUJOURS la mention « recommandé »,
      # même s'il est indisponible ; le modèle de repli effectivement utilisé
      # est alors marqué « utilisé en référence ».
      if (identical(k, ref_key)) paste0(lab, " (recommandé)")
      else if (identical(k, ref_utilise)) paste0(lab, " (utilisé en référence)")
      else lab
    }
    ...
    if (!calc_fait) {
      ...
      return(DT::datatable(df_empty,
        container = sketch,
        # Pas de sélection de ligne (2026-09 v4) : un clic ne surligne plus la
        # ligne en bleu ; seules les cases « Comparer » sont interactives.
        selection = "none",
        options   = opts_base,
        rownames = FALSE,
    ...
    tt_shuter <- "Modèle empirique calibré sur la truite grise en Ontario (Shuter et coll. 1998). Régression log-log reliant le rendement à la superficie et aux solides dissous totaux (SDT). Intrants : superficie, SDT (depuis la conductivité)."
    ...
    tt_boivin_omble <- paste0(
      "Vaillancourt et Boivin 2000 (pourvoiries du Saguenay–Lac-Saint-Jean). QUOTA DE DÉPART fixé par une table de codes, et non rendement maximal : le taux de 80 % ne s'applique pas. ",
      "Premier code applicable dans l'ordre du document : absence de tributaire (0), superficie < 2 ha (non considéré), 2 à 5 ha, > 5 ha sans profondeur moyenne, profondeur moyenne < 1,5 m, 1,5 à 2,5 m, puis 70 % de Vézina. Valeurs réduites si le meunier est présent (21 % de Vézina). ",
      "Deux situations d'espèces seulement (omble seul, ou avec meunier) ; toute autre espèce rend le modèle indisponible. ",
      "Intrants : superficie, espèces présentes, tributaire/émissaire, profondeur moyenne (facultative)."
    )
    ...
    note_boivin_omble <- if (!is.null(r$omble_boivin) && !is.na(r$omble_boivin$rendement_ha)) {
      r$omble_boivin$note
    } else if (!is.null(r$omble_boivin)) paste0("\u26a0 ", r$omble_boivin$note) else ""
    ...
    tip_lib  <- list(lester = tt_lester, shuter = tt_shuter, ime = tt_ime,
                     ...
                     omble_valin = tt_valin_omble, omble_boivin = tt_boivin_omble)
    note_lib <- list(lester = note_lester, shuter = note_shuter, ime = note_ime,
                     ...
                     omble_valin = note_valin_omble, omble_boivin = note_boivin_omble)
    ...
    nom_mod   <- vapply(keys_mod, function(k)
      tip_mod(label_mod(k), tip_lib[[k]],
              gras = identical(k, ref_key) || identical(k, ref_utilise)), character(1))
    ...
    note_modele <- function(k) {
      ...
      court <- switch(k,
        ...
        omble_boivin = "Quota de départ, sans taux de 80 %",
        "")
    ...
    # Exception (2026-09) : un modele « quota de depart » (quota_depart = TRUE)
    # fournit deja un quota -> facteur 1, pas PCT_RECOMMANDE. Un facteur par
    # ligne, dans l'ordre de all_vals (modeles puis grilles regionales).
    fact_reco <- c(vapply(keys_mod, function(k)
                     if (isTRUE(config()$modeles[[k]]$quota_depart)) 1 else PCT_RECOMMANDE / 100,
                     numeric(1)),
                   rep(PCT_RECOMMANDE / 100, length(reg_vals)))
    fmt_qn_reco <- function(v) fmt_int(v * fact_reco * sup)
    ...
    df <- data.frame(
      "Comparer"                  = c(vapply(keys_mod, function(k)
                                        case_cmp(paste0("mod:", k)), character(1)),
                                      vapply(reg_keys, function(k) case_cmp(paste0("reg:", k)),
                                             character(1))),
    ...
    dt_out <- DT::datatable(df,
      escape    = FALSE,
      container = sketch,
      selection = "none",   # voir état vide ci-dessus
  ...
  output$table_note <- renderUI({
    if (isTRUE(calc_valide_rv()))
      tags$small(class = "text-muted fst-italic", "Cocher un modèle pour l'ajouter à la figure.")
  })
  ...
  output$export_hint_ui <- renderUI({
    ...
    n <- nrow(quotas_enregistres_rv())
    div(
      span(class = "export-compte", n),
      span(class = "export-texte",
           if (n == 0) "Aucun quota enregistré — « Enregistrer ce quota » dans la carte Quota révisé"
           else paste0("quota", if (n > 1) "s", " enregistré", if (n > 1) "s",
                       " dans la session · perdus à la fermeture, exporter avant de quitter")))
  })

  # Export Excel des quotas enregistrés dans la session (2026-09).
  # Dépendance : paquet writexl (léger, sans Java). Feuille « Quotas » : une
  # ligne par lac et par espèce ; feuille « Notes » : définitions des colonnes.
  output$dl_excel <- downloadHandler(
    filename = function() paste0("quotas_enregistres_", Sys.Date(), ".xlsx"),
    content = function(file) {
      tab <- quotas_enregistres_rv()
      if (nrow(tab) == 0) {
        tab <- data.frame(Message = "Aucun quota enregistré dans cette session.")
      } else {
        tab <- tab[order(tab[["Espèce"]], tab[["No lac"]]), setdiff(names(tab), c("cle", ".taux")),
                   drop = FALSE]
      }
      notes <- data.frame(
  ...
  observeEvent(input$lac_select, {
    ...
    for (id in c("nb_chalets_omble", "quota_actuel_touladi", "quota_actuel_dore",
                 "quota_actuel_omble", "lat_manuelle", "lon_manuelle"))
      updateNumericInput(session, id, value = NA)
    updateRadioButtons(session, "tributaire_emissaire_omble", selected = "inconnu")
    updateCheckboxGroupInput(session, "especes_presentes_touladi", selected = character(0))
    updateCheckboxGroupInput(session, "especes_presentes_dore",    selected = character(0))
