...
# -----------------------------------------------------------------------------
# COULEURS PAR ESPÈCE — CSS statique (révision 2026-09)
#
#   Avant : chaque changement d'onglet appelait session$setCurrentTheme(), qui
#   recompile le thème Bootstrap (Sass) côté serveur puis REMPLACE la feuille
#   de style dans le navigateur. Pendant ce remplacement, la page est
#   brièvement sans style — le « flash » de texte brut avant l'interface.
#   Maintenant : le thème Bootstrap reste fixe ; les trois palettes sont
#   déclarées ici en variables CSS, et le navigateur bascule une classe sur
#   <body> (esp-touladi, esp-dore, esp-omble) au clic, sans aller-retour
#   serveur ni rechargement de feuille de style. Les éléments recolorés sont
#   ceux que modifiaient setCurrentTheme() et l'ancien output$dynamic_theme_css.
# -----------------------------------------------------------------------------
css_especes <- local({
  vars <- function(p) sprintf(
    "--esp-primaire:%s; --esp-accent:%s; --esp-bar:%s; --esp-tint:%s;",
    p$primaire, p$accent, p$bar, p$tint)
  paste0(
    # Palette par défaut = espèce active au démarrage (aucun flash au chargement)
    ":root { ", vars(COULEURS[[ESPECE_ACTIVE]]), " }\n",
    paste0(vapply(names(COULEURS), function(k)
      sprintf("body.esp-%s { %s }", k, vars(COULEURS[[k]])), character(1)), collapse = "\n"),
    "
  .app-header  { background: linear-gradient(135deg, var(--esp-primaire), var(--esp-accent)) !important; }
  .species-bar { background: var(--esp-bar) !important; }
  .kpi-top-card.main { background: var(--esp-tint) !important; border-left-color: var(--esp-accent) !important; }
  .kpi-top-card.main .kpi-eyebrow { color: var(--esp-accent) !important; }
  .cond-block, .bloc-gauche { border-left-color: var(--esp-accent) !important; }
  #kpi_obs_n .form-check-input:checked + .form-check-label {
    background: var(--esp-accent) !important; border-color: var(--esp-accent) !important; color: white !important;
  }
  .btn-primary {
    --bs-btn-bg: var(--esp-primaire) !important; --bs-btn-border-color: var(--esp-primaire) !important;
    --bs-btn-hover-bg: var(--esp-accent) !important; --bs-btn-hover-border-color: var(--esp-accent) !important;
    --bs-btn-active-bg: var(--esp-accent) !important; --bs-btn-active-border-color: var(--esp-accent) !important;
    --bs-btn-disabled-bg: var(--esp-primaire) !important; --bs-btn-disabled-border-color: var(--esp-primaire) !important;
  }
  .form-check-input:checked { background-color: var(--esp-primaire) !important; border-color: var(--esp-primaire) !important; }
  .form-control:focus, .form-select:focus, .selectize-input.focus { border-color: var(--esp-accent) !important; }
  .progress-bar { background-color: var(--esp-primaire) !important; }
  .app-header, .species-bar, .bloc-gauche, .kpi-top-card.main { transition: background-color .2s, border-color .2s; }
")
})
...
css_minimal <- "
  ...
  /* ── Panneau gauche en 4 blocs (révision 2026-09) ──
     ...
     (recoloré par css_especes, selon la classe de <body>). */
  .bloc-gauche {
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML(css_especes)),
    # Bascule d'espèce côté navigateur : couleurs et onglet actif changent
    # immédiatement, puis le serveur est informé (input$espece_click).
    tags$script(HTML("
      window.choisirEspece = function(cle) {
        var b = document.body;
        ['touladi', 'dore', 'omble'].forEach(function(k) { b.classList.remove('esp-' + k); });
        b.classList.add('esp-' + cle);
        document.querySelectorAll('.sp-tab').forEach(function(t) {
          ['touladi', 'dore', 'omble'].forEach(function(k) { t.classList.remove('active-' + k); });
          if (t.getAttribute('data-espece') === cle) t.classList.add('active-' + cle);
        });
        Shiny.setInputValue('espece_click', cle, {priority: 'event'});
      };
    ")),
...
server <- function(input, output, session) {
  ...
  # Rendue une seule fois : l'onglet actif est ensuite basculé par le
  # navigateur (choisirEspece), sans re-rendu de la barre.
  output$species_bar <- renderUI(build_species_bar(isolate(espece_active_rv())))
  ...
  # (Recoloration : voir css_especes — plus de session$setCurrentTheme(),
  #  cause du flash de texte sans style au changement d'onglet.)
