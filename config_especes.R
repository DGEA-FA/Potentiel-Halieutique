...
build_species_bar <- function(active, disponibles = names(REGISTRE_ESPECES)) {
  onglet <- function(cle) {
    ...
    # choisirEspece() (défini dans app.R) bascule couleurs et onglet actif
    # dans le navigateur, puis informe le serveur — transition sans flash.
    div(
      class   = classe,
      title   = if (!dispo) "Aucune donnée pour le lac sélectionné" else NULL,
      `data-espece` = cle,
      onclick = sprintf("choisirEspece('%s')", cle),
