...
REGISTRE_ESPECES <- list(
  ...
  # ---------------------------------------------------------------------------
  # Omble de fontaine (Salvelinus fontinalis) — 4 modèles.
  ...
  #   - omble_boivin : Vaillancourt et Boivin 2000, table de codes donnant un
  #       QUOTA DE DÉPART (et non un rendement maximal). quota_depart = TRUE :
  #       jamais modèle de référence, taux de 80 % non appliqué. Placé en fin
  #       de cascade pour l'affichage et la comparaison seulement.
  ...
  omble = list(
    ...
    modeles  = list(
      ...
      omble_boivin = list(
        nom          = "Vaillancourt et Boivin 2000",
        fn           = calc_boivin_vaillancourt_omble,
        intrants     = c("A", "grp", "tributaire_absent",
                         "Dmn_optionnel", "Dmax_optionnel"),
        quota_depart = TRUE
      )
    ),
    cascade_reference = c("omble_valin", "archambault", "vezina", "omble_boivin")
  )
)
